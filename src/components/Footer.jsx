    ...
    return (
      ...
      `}</style>
            ...
            <motion.div
                ...
                className="df-card relative mx-auto max-w-6xl overflow-hidden rounded-[2rem] bg-gradient-to-b from-(--deep-a) to-(--deep-b) shadow-[0_20px_60px_-20px_rgba(0,0,0,0.8)]"
            >
                ...
                <div className="relative border-b border-(--border-soft) overflow-hidden py-3">
                ...
                <motion.div
                    ...
                    className="relative px-6 md:px-14 pt-16 pb-14 flex flex-col md:flex-row md:items-end md:justify-between gap-8 border-b border-(--border-soft)"
                >
                    <motion.h2
                        variants={item}
                        className="df-logo font-semibold text-3xl md:text-5xl leading-[1.05] max-w-xl"
                ...
                <motion.div
                    ...
                    className="relative grid grid-cols-2 md:grid-cols-4 gap-10 px-6 md:px-14 py-14 border-b border-(--border-soft)"
                >
                    {/* Brand column */}
                    <motion.div variants={item} className="col-span-2 md:col-span-1">
                        <div className="df-logo flex items-center gap-2.5 text-lg font-semibold text-(--text-strong) mb-4">
              <span className="relative w-3 h-3 inline-block">
                ...
                />
                        ...
                        <p className="df-nav text-sm text-(--text-soft) leading-relaxed max-w-[220px] mb-6">
                            {t('footer.tagline')}
                        </p>
                        <div className="flex items-center gap-2">
                            {SOCIALS.map(({ label, href, Icon }) => (
                                <motion.a
                                    ...
                                    className="w-9 h-9 rounded-full border border-(--border) flex items-center justify-center text-(--text-soft) hover:text-(--text-strong) hover:border-blue-400/40 hover:bg-blue-500/10 transition-colors duration-200"
                                >
                    ...
                    <motion.div variants={item}>
                        <p className="df-nav text-xs uppercase tracking-[0.2em] text-(--text-muted) mb-5">{t('footer.sitemapTitle')}</p>
                        <ul className="df-nav flex flex-col gap-3">
                            {SITEMAP.map((link) => (
                                <li key={link.key}>
                                    <a href={link.href} className="group inline-flex items-center gap-1.5 text-sm text-(--text-soft) hover:text-(--text-strong) transition-colors duration-200">
                    ...
                    {/* Studio column */}
                    <motion.div variants={item}>
                        <p className="df-nav text-xs uppercase tracking-[0.2em] text-(--text-muted) mb-5">{t('footer.studioTitle')}</p>
                        <ul className="df-nav flex flex-col gap-3">
                            {STUDIO.map((link) => (
                                <li key={link.key}>
                                    <a href={link.href} className="group inline-flex items-center gap-1.5 text-sm text-(--text-soft) hover:text-(--text-strong) transition-colors duration-200">
                    ...
                    {/* Newsletter column */}
                    <motion.div variants={item} className="col-span-2 md:col-span-1">
                        <p className="df-nav text-xs uppercase tracking-[0.2em] text-(--text-muted) mb-5">{t('footer.newsletterTitle')}</p>
                        <form
                            onSubmit={(e) => e.preventDefault()}
                            className="relative flex items-center rounded-full border border-(--border) bg-(--soft-bg) focus-within:border-blue-400/40 transition-colors duration-300 pl-4 pr-1 py-1"
                        >
                            <input
                                type="email"
                                placeholder={t('footer.newsletterPlaceholder')}
                                className="df-nav bg-transparent outline-none text-sm text-(--text-strong) placeholder:text-(--text-muted) w-full py-1.5"
                            />
                            <motion.button
                                type="submit"
                ...
                <div className="relative flex flex-col-reverse md:flex-row items-center justify-between gap-4 px-6 md:px-14 py-6">
                    <motion.p
                        ...
                        className="df-nav text-xs text-(--text-muted)"
                    >
                        © 2026 Dualis Team. {t('footer.rights')}
                    </motion.p>
                    ...
                    <motion.button
                        ...
                        whileHover={{ y: -3 }}
                        whileTap={{ scale: 0.95 }}
                        className="df-nav group flex items-center gap-2 text-xs text-(--text-soft) hover:text-(--text-strong) transition-colors duration-200"
                    >
                        {t('footer.backToTop')}
                        <span className="w-7 h-7 rounded-full border border-(--border) flex items-center justify-center group-hover:border-cyan-300/40 group-hover:bg-cyan-500/10 transition-colors duration-200">
              <motion.span
                  animate={{ y: [0, -3, 0] }}
